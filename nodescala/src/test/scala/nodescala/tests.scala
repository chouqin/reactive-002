...
//  test("another version of cancellation") {
...
//        println("working")
  ...
  test("Future.continueWith should handle exceptions thrown by the user specified continuation function") {
    val f = Future.always("hello") continueWith {
      f => throw new Exception("user specified exception")
    }
    try {
      ...
      case t : Exception => assert(t.getMessage == "user specified exception")
    }
  }
