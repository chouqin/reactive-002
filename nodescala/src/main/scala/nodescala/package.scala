...
//      p.future
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      Try {
        Await.result(f, Duration(0, "second"))
      } match {
        case Failure(_: TimeoutException) => throw new NoSuchElementException
